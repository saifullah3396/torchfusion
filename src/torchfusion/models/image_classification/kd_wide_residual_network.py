...
def conv3x3(in_channels, out_channels, stride=1):
    # convenient shortcuts for constructing the network
    return nn.Conv2d(
        in_channels, out_channels, kernel_size=3, padding=1, bias=False, stride=stride
    )
...
class WideResidualNetwork(nn.Module):
    ...
    :param int num_labels: number of classes in the classification dataset.
    :param float drop_rate: dropout ratio for the dropout layers.
    """

    def __init__(self, depth, width, num_labels=10, drop_rate=0.0):
        super(WideResidualNetwork, self).__init__()
        self.depth = depth
        self.width = width
        self.layerwise_num_channels = [
            int(16 * width),
            int(32 * width),
            int(64 * width),
        ]
        ...
        self.fc = nn.Linear(self.layerwise_num_channels[2], self.num_labels)
...
class TeacherWideResidualNetwork(WideResidualNetwork):
    ...
    def __init__(self, depth, width, num_labels=10, drop_rate=0.0, load_path=None):
        super(TeacherWideResidualNetwork, self).__init__(
            depth, width, num_labels, drop_rate
        )
...
class StudentWideResidualNetwork(WideResidualNetwork):
    ...
    def __init__(self, depth, width, teacher_width, num_labels=10, drop_rate=0.0):
        super(StudentWideResidualNetwork, self).__init__(
            depth, width, num_labels, drop_rate
        )
        ...
        self.branch1 = nn.Sequential(
            conv1x1(
                self.layerwise_num_channels[0], self.teacher_layerwise_num_channels[0]
            ),
            conv1x1(
                self.teacher_layerwise_num_channels[0],
                self.teacher_layerwise_num_channels[0],
            ),
        )
        self.branch2 = nn.Sequential(
            conv1x1(
                self.layerwise_num_channels[1], self.teacher_layerwise_num_channels[1]
            ),
            conv1x1(
                self.teacher_layerwise_num_channels[1],
                self.teacher_layerwise_num_channels[1],
            ),
        )
        self.branch3 = nn.Sequential(
            conv1x1(
                self.layerwise_num_channels[2], self.teacher_layerwise_num_channels[2]
            ),
            conv1x1(
                self.teacher_layerwise_num_channels[2],
                self.teacher_layerwise_num_channels[2],
            ),
        )
        ...
        self.variance_param1 = nn.Parameter(
            torch.full(
                (self.teacher_layerwise_num_channels[0], 1, 1),
                init_variance_param_value,
            )
        )
        self.variance_param2 = nn.Parameter(
            torch.full(
                (self.teacher_layerwise_num_channels[1], 1, 1),
                init_variance_param_value,
            )
        )
        self.variance_param3 = nn.Parameter(
            torch.full(
                (self.teacher_layerwise_num_channels[2], 1, 1),
                init_variance_param_value,
            )
        )
    ...
    def forward(self, x):
        ...
        return out, (
            (branch1_out, variance1),
            (branch2_out, variance2),
            (branch3_out, variance3),
        )
