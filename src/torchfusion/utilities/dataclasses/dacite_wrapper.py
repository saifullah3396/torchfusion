...
from typing import Any, Mapping, Optional, Set, Type, TypeVar, get_type_hints

from dacite.config import Config
from dacite.core import extract_init_var, is_dataclass, is_init_var, is_union
...
class UnexpectedDataErrorCustom(UnexpectedDataError):
    def __init__(self, dataclass, keys: Set[str]) -> None:
        super().__init__(keys)
        self.dataclass = dataclass

    def __str__(self) -> str:
        formatted_keys = ", ".join(f'"{key}"' for key in self.keys)
        return f"can not match {formatted_keys} to any [{self.dataclass.__name__}] data class field"
...
def from_dict(data_class: Type[T], data: Data, config: Optional[Config] = Config(strict=True)) -> T:
    ...
    if config.strict:
        ...
        if extra_fields:
            raise UnexpectedDataErrorCustom(dataclass=data_class, keys=extra_fields)
