...
class TestModels(AnalyzerTask):
    ...
    class Config(AnalyzerTaskConfig):
        ...
        checkpoint_keys: List[str] = field(default_factory=lambda: ['model', 'ema_model'])
    ...
    def run(self):
        ...
        for model_dir in self.config.model_dirs:
            base_checkpoint_dir = Path(self.config.base_dir) / Path(model_dir) / 'checkpoints'
            ...
            for checkpoint in files:
                for key in self.config.checkpoint_keys:
                    ...
                    logger.info(f"Testing model [{model_dir} with checkpoint [{checkpoint}]], checkpoint_key={key}")

                    # running checkpoint
                    self._tb_logger = FusionTensorboardLogger(self._output_dir / model_dir / checkpoint / key)
                    ...
                    self._visualization_engine = self._trainer_functionality.initialize_visualization_engine(
                        args=self._args,
                        model=model,
                    ...
                    # # run visualizer
                    self._visualization_engine.run(self._test_dataloader, max_epochs=1, epoch_length=1)
