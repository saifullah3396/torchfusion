...
def prepare_datasets(args: FusionArguments, hydra_config: OmegaConf):
    # initialize training
    initialize_torch(
        args,
    ...
    #     # for example
    #     type_map={DataKeys.IMAGE: torch.float32, DataKeys.LABEL: torch.long}
    # )
    ...
    print_tf_from_loader(train_dataloader, stage=TrainingStage.train)
    print_tf_from_loader(val_dataloader, stage=TrainingStage.validation)
    print_tf_from_loader(test_dataloader, stage=TrainingStage.test)
...
def main(
    ...
    try:
        return prepare_datasets(args, hydra_config)
